...
# This Section is for Report/ Order Wise Profite
@login_required
@daseboard_required
def calculate_product_profit(request):
    # Query all orders
    orders = Order.objects.all()
    # Dictionary to store profits by order id
    order_profits = {}

    # Iterate through each order
    for order in orders:
        # Initialize profit for the current order
        order_profits[order.id] = 0

        # Iterate through each order item in the order
        for order_item in order.items.all():
            product = order_item.item 

            # Calculate profit for the product in the order item
            product_profit = product.price - product.product_purchase_price 
            order_profits[order.id] += product_profit
            order_item.product_profit = product_profit 
            print('###########################')
            print('product_profit:', product_profit)
            order_item.save()
        
    # Print total profit for each order
    for order_id, total_profit in order_profits.items():
        print(f"Order ID: {order_id}, Total Profit: {total_profit}")
    ...
    return render(request, 'dashboard/report/order_wise_profit.html', context)
